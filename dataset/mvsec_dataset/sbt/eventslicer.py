...
class EventSlicer:
    def __init__(self, h5f: h5py.File):
        ...
        for i, dataset_str in enumerate(['x', 'y', 't', 'p']):
            self.events[dataset_str] = self.h5f[f"davis/left/events"][:, i]
            if dataset_str == 't':
                #Float to int64 conversion
        ...
        # UPDATE: ms_to_idx not available in the dataset MVSEC: use np.searchsorted instead
        # self.ms_to_idx = np.asarray(self.h5f['ms_to_idx'], dtype='int64')
        self.ms_to_idx = np.searchsorted(self.events['t'], np.arange(self.events['t'][0] // 1000, (self.events['t'][-1] // 1000) + 1, 1) * 1000)
    ...
    def ms2idx(self, time_ms: int) -> int:
        time_ms = time_ms - self.t_start // 1000
        if time_ms < 0:
            time_ms = 0
        # assert time_ms >= 0
        if time_ms >= self.ms_to_idx.size:
            time_ms = self.ms_to_idx.size - 1
