...
def depth_evaluation(
    ...
    if metric_scale:
        ...
        s_init = torch.median(ground_truth_depth) / torch.median(predicted_depth)
    ...
    if custom_mask is not None:
        ...
        # Ensure mask and custom_mask are on the same device
        if mask.device != custom_mask.device:
            mask = mask.to(custom_mask.device)
